...
已有数据库应先执行 `database_migration_p_system.sql`（如尚未执行），再执行 `database_migration_approval_eportal.sql`、`database_migration_customer_id.sql` 和 `database_migration_eportal_users.sql`；全新安装直接使用 `database.sql`。
