...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($salesPerson === '') {
        ...
        try {
            ...
            $stmt = $pdo->prepare('INSERT INTO contract_forms (created_by, created_by_name, created_by_mail, sales_person, status, created_at, submitted_recognition_at, updated_at) VALUES (:user_id, :name, :mail, :sales_person, 1, NOW(), NULL, NOW())');
            $stmt->execute([':user_id' => (int)$_SESSION['user_id'], ':name' => $operator, ':mail' => (string)($_SESSION['email'] ?? ''), ':sales_person' => $salesPerson]);
