...
$error='';
$fields=['po_no','customer_name','customer_delivery_address','end_user_name','end_user_contact','end_user_email','vendor_part_no','description','qty','price_currency','unit_price'];
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if(count(array_unique(array_values($counts)))>1)$error='产品明细字段数量必须一致，请使用英文分号分隔。';
 if($error===''){
  try{
...
<?php foreach(['po_no'=>'PO No.','customer_name'=>'Customer Name','customer_delivery_address'=>'Customer Delivery Address','end_user_name'=>'End User Name','end_user_contact'=>'End User Contact','end_user_email'=>'End User Email'] as $field=>$label):?><div class="form-group <?=$field==='customer_delivery_address'?'full':''?>"><label><?=h($label)?></label><input class="form-control" name="<?=h($field)?>" value="<?=h($task[$field])?>"></div><?php endforeach;?>
