...
接口只接受状态为 `4`（匹配中）的任务。成功保存 `pid` 和 `p_sys_link` 后，系统会使用 `template/costing_sheet_v1.xlsx` 立即生成 Costing Sheet；生成成功后任务状态更新为 `6`（待审批）。如果生成失败，任务保持状态 `5`（待建表），可调用 `POST /api/costing_sheet.php` 并传入 `task_id` 重试。
